...
class Position(abstract_broker.AbstractPosition):
    """
...
# class CbproAccount(abstract_broker.AbstractBrokerAccount):
class AccountInfo:
...
def cbpro_init_stream(symbols: t.List[str], creds):
    stream = CbproStream(**creds).stream
...
class CbProTickerStream(AbstractTickerStream):
    ...
    def run_stream(
        ...
        :param writer_send_conn:
        :param stream_generator: function returning a stream client
        :param symbols: symbols to stream
        :return:
        """
        self._init_stream_parsers(symbols, datetime.utcnow())
        send_conn = self._init_processes(writer_send_conn)
        ...
        while True:
            ...
            if symbol is not None and msg["price"] != cached_msgs[symbol]:
                ...
                self._stream_parsers[symbol].update_ohlc(msg, datetime.utcnow())
